...
export type ReportKind = 'full' | 'products' | 'categories' | 'customers' | 'invoices'
...
function renderInvoices(doc: jsPDF, b: ReportBundle, y: number): number {
  const total = b.invoices.reduce((s, inv) => s + Number(inv.total_amount || 0), 0)
  y = sectionTitle(doc, 'Invoice Register', `${b.invoices.length} invoices  •  ${money(total, b.currency)}`, y)
  autoTable(doc, {
    startY: y,
    head: [['Invoice #', 'Date', 'Customer', 'Type', 'Total', 'Paid', 'Status']],
    body: b.invoices.slice(0, 300).map((inv) => [
      inv.invoice_number,
      new Date(inv.created_at).toLocaleDateString(),
      inv.customer_name || 'Walk-in',
      inv.invoice_type || 'pos',
      money(inv.total_amount, b.currency),
      money(inv.payment_status === 'paid' ? inv.total_amount : inv.amount_paid, b.currency),
...
export async function exportReportPdf(kind: ReportKind, b: ReportBundle) {
  ...
  if (kind === 'products') {
    ...
    y = renderCustomers(doc, b, y)
    renderInvoices(doc, b, y)
  }
    ...
    * { box-sizing: border-box; }
    ...
    <div class="section">
      ...
      ${printTableHtml(
        ...
        b.invoices.slice(0, 150).map((inv) => [inv.invoice_number, new Date(inv.created_at).toLocaleDateString(), inv.customer_name || 'Walk-in', money(inv.total_amount, currency), inv.payment_status || 'unpaid']),
        [3]
      )}
    </div>
...
export async function exportReportExcel(kind: ReportKind, b: ReportBundle) {
  ...
  if (kind === 'full' || kind === 'invoices') {
    mkSheet(
      'Invoices',
      ['Invoice #', 'Date', 'Customer', 'Type', 'Total', 'Paid', 'Status'],
      b.invoices.map((inv) => [
        inv.invoice_number,
        new Date(inv.created_at).toISOString().slice(0, 10),
        inv.customer_name || 'Walk-in',
        inv.invoice_type || 'pos',
        Math.round(Number(inv.total_amount) * 100) / 100,
        Math.round(Number(inv.payment_status === 'paid' ? inv.total_amount : inv.amount_paid) * 100) / 100,
        inv.payment_status || 'unpaid',
      ])
    )
  }
